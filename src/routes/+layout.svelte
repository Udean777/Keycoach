<script lang="ts">
  ...
  import { t, langs, getLang, setLang, subscribeLang } from '$lib/i18n';
...
<div class="flex min-h-screen flex-col">
  ...
  <header class="sticky top-0 z-40 border-b border-[var(--color-border)] bg-[var(--color-bg)]/85 backdrop-blur supports-[backdrop-filter]:bg-[var(--color-bg)]/70">
    <div class="mx-auto flex min-h-14 w-full max-w-4xl items-center justify-between gap-4 px-4 py-2">
      ...
      <nav class="flex items-center gap-1">
        {#each links as link (link.href)}
          <a
            href={link.href}
            class="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold transition-colors {page.url.pathname === link.href ? 'bg-[var(--color-surface)] text-[var(--color-accent)]' : 'text-[var(--color-muted)] hover:text-[var(--color-ink)]'}"
          >
      ...
      <select
        value={currentLang}
        onchange={(e) => {
          const lang = e.currentTarget.value as 'en' | 'id';
          setLang(lang);
          srs.load(lang);
        }}
  ...
  <footer class="border-t border-[var(--color-border)] py-6">
    <div class="mx-auto flex w-full max-w-4xl flex-col items-center justify-between gap-2 px-4 text-xs text-[var(--color-muted)] sm:flex-row">
